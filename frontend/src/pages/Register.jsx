...
const Register = () => {
  ...
  const handleRegister=async ()=>{
    ...
    try{
      ...
      setError(false)
      navigate("/login")
    //   if successful navigate to login page an no error put error to false
    }
    catch(err){
      setError(true)
      console.log(err)
  ...
  return (
    <>
    {/* to show login and blog market logo in login page {/* this first part is for custom nav bar for (for login page mention register) */} 
    ...
    <div className="w-full flex justify-center items-center h-[80vh] ">
       <div className="flex flex-col justify-center items-center space-y-4 w-[80%] md:w-[25%]">
         ...
         {/* on chane event re changes the input value as user gives */}
         ...
         {/* if any error occurs in register */}
         {error && <h3 className="text-red-500 text-sm ">Something went wrong</h3>}
         {/* if have an account login go to */}
